...
const SideMenu: FC<SidebarProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div
      className={`fixed inset-y-0 h-screen left-0 z-50 w-full text-white/70 bg-black/50 shadow-xl ${isOpen ? "translate-x-0" : "-translate-x-full"} hoverEffect`}
    >
      <div
        ref={sidebarRef}
      ...
      >
        ...
        <div className="flex-col flex space-y-3.5 font-semibold tracking-wide">
          {headerData?.map((item) => (
            <Link
              href={item?.href}
              key={item?.title}
